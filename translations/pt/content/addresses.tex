...
{\tt construct\_ tx\_ with\_ tx\_ key()}}[-6cm]
...
crypto- note\_ tx\_ utils.cpp}[-6cm]
...
\begin{itemize}
 ...
 alinhada com uma saída de endereço principal é \(R_t=r_tG\), embora essa saída
 continue a usar \(R\) na derivação. Para uma saída de sub-endereço que não seja
 troco, é \(R_t=r_tK_t^{s,\iota}\), e a derivação usa \(R_t\).
\end{itemize}
...
Numa saída de endereço principal, o segredo partilhado continua a ser
\(8rK_t^v=8k_t^vR\). Numa saída de sub-endereço que use \(R_t\), é
\(8r_tK_t^{v,\iota}=8k_t^vR_t\). É a derivação correspondente que entra em
\(h_t\) e na etiqueta de vista da saída.
