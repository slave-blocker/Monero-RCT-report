...
%The command {\tt print\_tx} would report the block where it found the transacção, which we replicate here for demonstration purposes.
		\item {\tt version} (linha 4) -- A versão de serialização da
		transacção; `2' é a versão usada pelos formatos RingCT, históricos e
		actuais.
%Transacção format/era version; `2' corresponds to RingCT.
		\item {\tt unlock\_time} (linha 5) -- Impede que as saídas sejam
		gastas antes do limite comum. Um valor inferior a \(500\,000\,000\) é
		uma altura de bloco; um valor a partir daí é um tempo Unix. Zero não
		acrescenta bloqueio.
...
%List of inputs (there are two here)
		\item {\tt amount} (linha 8) -- Campo de montante visível. É zero
		nestas entradas RingCT porque o montante está comprometido.
%Deprecated (legacy) amount field for type 1 transactions
		\item {\tt key\_offsets} (linha 9) -- Permitem encontrar na lista de
		blocos as chaves e os compromissos dos membros do anel. O primeiro
		índice é absoluto e os seguintes são diferenças relativas. Por exemplo,
		\(\{7,11,15,20\}\) serializa-se como \(\{7,4,4,5\}\), pois
		\(7+4+4+5=20\). Cada lista deste exemplo tem onze elementos, o tamanho
		histórico; na versão 16 teria dezasseis (secção
		\ref{subsec:space-and-ver-rcttypefull}).
%This allows verifiers to find ring member keys and commitments in the blockchain, and makes it obvious those members are legitimate. The first offset is absolute within the blockchain history, and each subsequent offset is relative to the previous. For example, with real offsets \{7,11,15,20\}, the blockchain records \{7,4,4,5\}. Verifiers compute the last offset like (7+4+4+5 = 20) (Section \ref{subsec:space-and-ver-rcttypefull}).
		\item {\tt k\_image} (linha 12) -- Imagem de chave da primeira
		entrada. Neste exemplo é a imagem ligada pela MLSAG; nas transacções
		actuais, a mesma posição do prefixo contém a imagem ligada pela CLSAG.
...
%List of outputs (there are two here)
		\item {\tt amount} (linha 25) -- Campo de montante visível, zero numa
		saída RingCT ordinária.
		\item {\tt key} (linha 27) -- Chave do endereço oculto de destino para
		a saída \(t=0\), como descrito na secção
		\ref{sec:one-time-addresses}. O alvo histórico não tem o \emph{view
		tag} que passou a ser obrigatório nas saídas v16.
%One-time destination key for output $t = 0$ as described in Section \ref{sec:one-time-addresses}
	\item {\tt extra} (linhas 36--39) -- Dados convencionais da carteira.
\marginnote{\footnotesize\path{src/cryptonote_basic/tx_extra.h}} Cada número
mostrado é
um byte. A etiqueta `1' anuncia a chave pública de transacção \(rG\), de 32
bytes, que permite ao destinatário derivar o segredo partilhado da secção
\ref{sec:one-time-addresses}; por ter tamanho fixo, não leva um comprimento.
Segue-se a etiqueta `2', um \emph{nonce} de nove bytes: o primeiro, `1',
identifica um ID de pagamento cifrado, e os oito restantes contêm-no. As
etiquetas dão significado convencional aos bytes; a fronteira de validação
actual é explicada no capítulo \ref{chapter:transactions}. Vejam-se
...
%First part of signature data
		\item {\tt type} (linha 41) -- Tipo RingCT; neste exemplo,
		{\tt RCTTypeBulletproof2} é `4'. {\tt RCTTypeFull} e
		{\tt RCTTypeSimple} são `1' e `2'; as transacções de mineiro usam
		{\tt RCTTypeNull}, `0'. O caminho ordinário v16 usa
		{\tt RCTTypeBulletproofPlus}, `6'.
...
%Second part of signature data
    \item {\tt nbp} (linha 52) -- Número de provas de domínio
    \emph{Bulletproof} neste exemplo. A prova única cobre as duas saídas.
...
%Bulletproof proof elements (Bulletproofs were not explored in this document, so we will not itemize further)
    ...
    \item {\tt MGs} (linhas 81-129) -- Duas assinaturas MLSAG, uma por
    entrada. Uma transacção v16 teria o vector {\tt CLSAGs}.
%MLSAG signatures
...
%from aforementioned MLSAG signature
    \item {\tt pseudoOuts} (linhas 130-131) -- Pseudo-compromissos de saída
    \(\widetilde C^a_j\), como descrito na secção
    \ref{sec:pedersen_monero}. A sua soma é igual à soma dos dois compromissos
    de saída mais o compromisso da taxa \(fH\).
...
Este apêndice mostra a estrutura do bloco de génese. O bloco não contém
transacções normais; contém apenas a transacção de mineiro que atribui a
primeira subvenção a thankful\_for\_today \cite{bitmonero-launched}. O campo de
carimbo temporal tem valor zero. O código inicial de Monero derivou de
Bytecoin, cuja história é descrita em \cite{monero-history,bytecoin-network}.
