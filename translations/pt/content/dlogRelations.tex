...
\chapter{Relações de logaritmo discreto entre geradores}
...
Estas relações são por vezes designadas «chaves privadas dos geradores». É
necessário distinguir dois casos. Uma
...
\]
A segunda relação pode ser conhecida sem fornecer o logaritmo entre um par
escolhido, porque os restantes termos continuam presentes. A hipótese
certa é, pois, a \emph{vinculação por logaritmo discreto generalizado}, não
uma colecção de «três gammas secretos».

Também importa \emph{quem} conhece a relação. Uma relação deliberadamente
pública pode ser parte do protocolo; uma relação conservada em segredo pelo
criador dos parâmetros é uma armadilha; uma descoberta posterior torna a
relação pública daquele momento em diante. Esta última não extrai
retroactivamente todas as chaves ordinárias. Para calcular uma abertura
alternativa de um ponto continua geralmente a ser necessária uma abertura
inicial.
...
inverso, o caminho pode ser percorrido para trás, e os rótulos dos caminhos
são os produtos dos escalares das arestas.
Cada componente ligada entrega todos os logaritmos aos pares \emph{dentro}
dela. No desenho, porém, \(G\) e \(V\) continuam em componentes desligadas:
conhecer a aresta \(H-T\) não revela \(\log_GH\). Uma relação de aridade
superior é representada por uma hiper-aresta.

Convém ainda separar as consequências. Neste capítulo usaremos cinco
classificações:
...
intervalo falsa e inflação são resultados distintos. A invalidação de uma
hipótese de segurança não demonstra automaticamente uma exploração concreta.
...
\clearpage
\section{Relações que envolvem montantes e saídas}
...
\(O=xG\), a árvore conserva também
\(I=\mathcal H_p(O_{\rm original})\), onde \(O_{\rm original}\) é a chave
publicada antes da eventual limpeza de torsão, e a SAL publica
...
\subsection*{Relações de \(H\) com \(T\), \(U\) e \(V\)}
...
\(\mathcal H_p(O_{\rm original})\) não é um sexto parâmetro estático. Se,
para uma saída histórica, alguém descobre
\[
 \mathcal H_p(O_{\rm original})=\eta_OG,\qquad O=xG,\qquad
 L=x\mathcal H_p(O_{\rm original}),
\]
então
\[
 \boxed{L=\eta_OO.}
\]
O marcador da saída passa a ser calculável a partir de dados públicos. Conhecer
\(\eta_O\) para uma saída permite reconhecê-la quando \(L\) aparece; conhecer
as relações para todas permite testar todas as saídas da cadeia. Não revela \(x\), não assina
uma transacção e não faz \(L\) deixar de repetir.

\textbf{Conclusão --- falha só de privacidade:} identificação retroactiva da
saída gasta; sem roubo, quebra da autorização ou inflação demonstrada.

\clearpage
\section{Consequências de relações entre as bases da SAL}
\scriptsize

O código corrente não se limita a \(\widetilde I\) e \(R_{\rm fcmp}\). Para
uma abertura \(\widetilde O=xG+yT\), escreve, com \(z=xr_i\),
\begin{align*}
 \widetilde I&=I+r_iU,&
...
\end{align*}
Aqui \(r_p'=r_p-y-r_j\).
A prova de produto vincula \(z=xr_i\) a \(P\); três provas de
representação vinculam \(x\) a \(\widetilde O\), \(z\) à terceira linha e
\((x,z)\) a \(L\). A pertença FCMP vincula o mesmo \(r_i\) a
\(\widetilde I\) e \(R_{\rm fcmp}\). Cada ponto público impõe a igualdade das
variáveis partilhadas entre as provas.
...
\]
que o dono pode variar entre gastos da mesma folha. A relação conhecida não muda
\(r_i\) em \(\widetilde I=I+r_iU\); muda a interpretação do coeficiente de
\(U\) na representação conjunta nas bases \(U,T\).
...
usa \(r_i'=r_i+\Delta\) no produto \(z'=xr_i'\); o mesmo ponto \(R_{\rm fcmp}\) admite as duas aberturas. O termo extra em \(V\) é absorvido
pela máscara em \(T\) de \(P\). Resulta
\(L=x\widetilde I-xr_i'U\), variável com \(\Delta\).

\textbf{Conclusão --- exploração algébrica directa:} perdeu-se a vinculação do compromisso a \(r_i\) que compunha pertença e SAL; o dono pode produzir
marcadores diferentes.

\subsection*{\(V=qU\): inconsistência entre produto e ligação}
...
\subsection*{\(U=qG\) e \(V=qG\) consideradas isoladamente}
...
\subsection*{Relação multibase geral}
...
ataques acima mostram relações cujo suporte elimina a vinculação necessária. Uma relação
de aridade superior não deve ser promovida automaticamente a inflação, nem
ignorada por não fornecer isoladamente \(\log_GT\).

\begin{landscape}
\section{Análise das dez relações por pares}
...
\footnotesize
«Existentes» não quer dizer que um estranho possa escolher uma saída de
terceiro. Quer dizer que, depois da descoberta, o titular que ainda conserva a abertura
...
parar, actualizar regras ou rejeitar a família comprometida. As provas antigas permanecem registadas.
\end{landscape}

\footnotesize
\section{Consequências para as provas de intervalo}
...
\]
Uma abertura vectorial deixa de fixar os bits, os vectores de produtos ou as
máscaras vinculadas pelas equações de produto interno. É a perda de vinculação
de Pedersen aplicada a um compromisso multibase
\cite{Bulletproofs_paper}.

Isto não implica que qualquer relação vectorial permita criar valor. Uma
...
A relação \(G-H\) fornece o primeiro mecanismo directo. Uma prova de
...
falhas de parâmetros. A solidez vem da prova de conhecimento e das restantes restrições; as relações
públicas entre os múltiplos fazem parte da declaração. \textbf{Conclusão ---
relação inofensiva ou deliberadamente conhecida.}

\clearpage
\section{Consequências para Curve Trees, Selene e Helios}

Cada curva tem a sua própria família
\[
 g, h, \boldsymbol g, \boldsymbol h, J_{\rm init}.
\]
Selene e Helios são grupos distintos: uma relação dentro de uma curva não se
estende à outra. Em cada uma, os compromissos da prova têm
a forma
\[
...
\]
As famílias são derivadas de rótulos separados, sem configuração confiada, na
...
\]
Se essa colisão troca um filho honesto por outro compromisso, pode produzir
uma passagem falsa numa camada e propagá-la até à raiz. Relações
...
O caso de \(J_{\rm init}\) deve ser tratado separadamente. Se o seu coeficiente é sempre exactamente
...
Conhecer apenas um caminho verdadeiro permite provar a pertença do membro
correspondente, que é a propriedade pretendida.
...
Pertença falsa, isoladamente, não é autorização. Provar a pertença de uma
saída alheia não revela \(x\) e não produz a SAL. Para chegar à inflação, o
atacante constrói uma tupla que ele próprio sabe abrir, atribui-lhe um
...
\emph{exploração provável que exige composição}: relação utilizável em
...
camada Selene pode bastar sem uma segunda relação Helios, e vice-versa; uma
relação que apenas reabre um compromisso auxiliar mas não altera uma restrição relevante
...
\(G,H\) & \(H=\gamma G\) & compromisso de montante & não & não & não & não & não & sim
& abertura e transacção normal & alta: álgebra e código RingCT\\ \hline
\(G,T\) & \(T=\tau G\) & abertura \(x,y\) da saída & não & não & não & não & sim & condicional
& abertura própria; segundo gasto & alta: álgebra SAL/código\\ \hline
\(G,U\) & \(U=qG\) & \(x,z\) dentro de \(P\) & não & não & não & não & condicional & condicional
& outra relação que vença \(O,P'\) & média: álgebra; exploração não demonstrada\\ \hline
\(G,V\) & \(V=qG\) & \(x,r_i\) dentro de \(P\) & não & não & não & não & condicional & condicional
& outra relação que vença \(O,R\) & média: álgebra; exploração não demonstrada\\ \hline
\(H,T\) & \(H=qT\) & nenhuma equação isolada & não & não & não & não & não & condicional
& caminho \(T\leadsto G\) & alta: grafo e equações separadas\\ \hline
\(H,U\) & \(H=qU\) & nenhuma equação isolada & não & não & não & não & não & condicional
& caminho \(U\leadsto G\) & alta: grafo e equações separadas\\ \hline
\(H,V\) & \(H=qV\) & nenhuma equação isolada & não & não & não & não & não & condicional
& caminho \(V\leadsto G\) & alta: grafo e equações separadas\\ \hline
\(T,U\) & \(U=qT\) & \(zU+rT\) & não & não & não & não & sim & condicional
& abertura própria; segundo gasto & alta: deslocação SAL explícita\\ \hline
\(T,V\) & \(V=qT\) & \(R_{\rm fcmp}\) em \(r_i\) & não & não & não & não & sim & condicional
& abertura própria; segundo gasto & alta: deslocação SAL explícita\\ \hline
\(U,V\) & \(V=qU\) & \(r_iV+zU\) & não & não & não & não & sim & condicional
& abertura própria; segundo gasto & alta: deslocação SAL explícita\\ \hline
\(G,T,U,V\) & \(aG+bT+cU+dV=0\) & compromisso multibase & condi\-cional & não demonstrado & condi\-cional & não & condi\-cional & condi\-cional
& suporte deve vencer equações externas & média: vinculação certa; efeito depende do suporte\\ \hline
\(I\) & \(I=\eta_OG\) & bases da DLEQ tornam-se relacionadas & sim & não & não & não & não & não
& relação para a saída observada & alta: \(L=\eta_OO\)\\ \hline
BP/BP+ & relação em \(H,\boldsymbol G,\boldsymbol H\) & compromisso vectorial & não & não & não & condicional & não & condicional
& direcção altera testemunho; balanço & média: abertura explícita; falsificação depende da relação\\ \hline
Selene & relação em \(g,h,\boldsymbol g,\boldsymbol h,J_{\rm init}\) & nó ou compromisso da prova & não & não & condicional & não & não & condicional
& colisão/restrição útil; saída própria e SAL & média: Curve Trees e composição\\ \hline
Helios & relação em \(g,h,\boldsymbol g,\boldsymbol h,J_{\rm init}\) & nó ou compromisso da prova & não & não & condicional & não & não & condicional
& colisão/restrição útil; saída própria e SAL & média: Curve Trees e composição\\ \hline
Controlo & \(J,2J,\ldots;\ P=xG;\ R=rH\) & nenhuma & não & não & não & não & não & não
& prova de conhecimento normal & alta: relação deliberada\\ \hline
\end{tabular}
\end{center}

\scriptsize
O único «sim» directo na coluna de inflação é \(G-H\). As arestas SAL
...
privacidade sem autorização; uma hipótese invalidada continua a exigir
correcção mesmo sem uma exploração concreta demonstrada.
