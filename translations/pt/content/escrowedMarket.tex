...
A garantia não é um contrato executado pelo consenso do Monero. O consenso
...
Não implementada na revisão consultada &
A construção actual termina em CLSAG distribuída; a adaptação a FCMP++ exige
uma SAL distribuída, como indicado na secção
...
\(\mathbf v\) corresponde ao contrato comercial.
...
participantes podem detectar as saídas recebidas e observar a actividade da
carteira. A confidencialidade dos montantes perante observadores externos não
implica confidencialidade entre \(B\), \(V\) e \(M\).
...
privada completa. Essa integração não existe na revisão consultada. A
secção \ref{sec:multisig-fcmp-note} descreve as relações necessárias.
