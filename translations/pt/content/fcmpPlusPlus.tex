...
\begin{align*}
 \underbrace{\mathcal B_\ell=
   ...
   _{\substack{\text{um ramo: }m\text{ irmãos}\\
               \text{no mesmo nível }\ell}}
 \quad\xrightarrow{\ \operatorname{Com}_{\ell\to\ell+1}\ }\quad
 \underbrace{P^{(\ell+1)}}_{\text{um pai no nível }\ell+1}.
\end{align*}
Na terminologia da implementação, um ramo \(\mathcal B_\ell\) é a lista
ordenada dos filhos de \emph{um} pai; não é o nível inteiro nem o trajecto da
folha à raiz. Usaremos colchetes para escolher uma posição dessa lista:
\begin{align}
 \boxed{\mathcal B_\ell[i]:={}_{\ell}X_i}.
 \label{eq:fcmp-branch-slot}
\end{align}
Um caminho atravessa níveis: escolhe uma posição num ramo, leva o pai
resultante para o ramo seguinte e repete.
...
\end{align}
Aqui a comutatividade não apaga as posições. Já numa única coordenada se vê a
diferença:
\begin{align*}
 \text{reordenar as parcelas:}\quad
 {}_{\ell}x_0G_0^x+{}_{\ell}x_1G_1^x
 &= {}_{\ell}x_1G_1^x+{}_{\ell}x_0G_0^x,\\
 \text{permutar os filhos:}\quad
 {}_{\ell}x_0G_0^x+{}_{\ell}x_1G_1^x
 &\underset{\text{em geral}}{\ne}
 {}_{\ell}x_1G_0^x+{}_{\ell}x_0G_1^x.
\end{align*}
Na primeira linha, cada escalar continua emparelhado com o seu gerador; na
segunda, os filhos trocaram de posição. O mesmo vale para as coordenadas
\(y_j\). Portanto, \(\mathcal B_\ell\) é uma lista ordenada: depois de fixada a
raiz, não se baralha o ramo. A prova esconde \(i_\ell\), não reordena
\(\mathcal B_\ell\).
As somas percorrem os filhos do nível \(\ell\); o ponto que sai delas pertence
ao nível seguinte. Se o caminho continuar, esse mesmo ponto passa a ocupar a
posição \(i_{\ell+1}\) no pai de cima:
\begin{align}
 P^{(\ell+1)}={}_{\ell+1}X_i.
...
\end{align*}
Cada \(P_j\) compromete os três pontos \((O,I,C)\) de cada uma das quatro
folhas \(L_{4j},\ldots,L_{4j+3}\): duas coordenadas por ponto. A raiz
compromete as coordenadas dos quatro \(P_j\):
...
Se a folha secreta é \(L_6\), todo o caminho cabe em três igualdades:
\begin{align*}
 \mathcal B_0[2]
   &={}_{0}X_2=L_6=(O_6,I_6,C_6),\\
 \operatorname{Com}_{0\to1}(\mathcal B_0)
   &=\boxed{P_1}={}_{1}X_1=\mathcal B_1[1],\\
 \operatorname{Com}_{1\to2}(\mathcal B_1)
   &=\mathcal R_{\rm tree}.
\end{align*}
Cada linha usa um ramo local; o caminho é o encaixe vertical das três igualdades.
Uma prova não precisa de representar os restantes quinze caminhos. Precisa de
...
Acrescentar uma folha não manda o nó percorrer todas as anteriores. Se \(L_4\)
já ocupa o primeiro lugar do ramo activo, a chegada de \(L_5\) muda uma só
coluna:
\begin{align*}
\begin{array}{r@{\qquad}c@{\qquad}c@{\qquad}c@{\qquad}c@{\qquad}l}
 \text{posição global da folha}&4&\boxed{5}&6&7&\text{resumo}\\
 \mathcal B_0\text{ antes}&L_4&\square&\square&\square&\Longrightarrow P_1\\
 \mathcal B_0\text{ depois}&L_4&L_5&\square&\square&\Longrightarrow P_1'
\end{array}
\end{align*}
Os três lugares vazios correspondem, da esquerda para a direita, às posições
globais \(5,6,7\). Na linha ``antes'', \(L_5,L_6,L_7\) ainda não existem; na
linha ``depois'', apenas \(L_5\) passou a existir. Os \(\square\) não são
saídas fictícias usadas numa transacção: são apenas enchimento zero do
compromisso, contribuem \(0\mathbin{\cdot}G_j=\mathcal O\) e não podem ser
seleccionados nem gastos. A saída real substitui \(\square\) pela folha inteira
\(L_5=(O_5,I_5,C_5)\), não apenas por \(O_5\). O símbolo \(\square\) diz
``ainda vazio''; a codificação interna desse vazio depende da camada.

O novo resumo \(P_1'\) sobe, ainda sem deslocar ou baralhar nada:
\begin{align*}
\begin{array}{r@{\qquad}c@{\qquad}c@{\qquad}c@{\qquad}c@{\qquad}l}
 \text{posição dentro da raiz}&0&\boxed{1}&2&3&\text{raiz}\\
 \mathcal B_1\text{ antes}&P_0&P_1&\square&\square
   &\Longrightarrow\mathcal R_{\rm tree}\\
 \mathcal B_1\text{ depois}&P_0&P_1'&\square&\square
   &\Longrightarrow\mathcal R'_{\rm tree}
\end{array}
\end{align*}
Assim, o caminho recalculado lê-se directamente nas duas colunas em caixa:
\begin{align*}
 \boxed{\text{posição }5:\ \square\mapsto L_5}
 \quad\Longrightarrow\quad
 \boxed{\text{posição }1:\ P_1\mapsto P_1'}
 \quad\Longrightarrow\quad
 \boxed{\mathcal R_{\rm tree}\mapsto\mathcal R'_{\rm tree}}.
\end{align*}
\(P_0\) e todo o seu ramo continuam guardados e são reutilizados, sem serem
recalculados.

Pela linearidade, o compromisso guardado recebe apenas a diferença da posição
alterada. Na instanciação actual, uma saída nova fornece duas coordenadas para
cada um dos três pontos \(O,I,C\), e um rótulo do hash em cada camada superior.
Sem aumento de altura,
\begin{align*}
 \underbrace{2+2+2}_{O,\ I,\ C}
 +\underbrace{(d-1)}_{\text{um rótulo por camada superior}}
 &=d+5\quad\text{novos termos escalar--ponto},\\
 \boxed{\text{trabalho criptográfico incremental}}
 &=\Theta(d)=\Theta(\log_m N),
   \qquad\text{em vez de reconstruir }\Theta(N).
\end{align*}
Cada subida acrescenta ainda uma adição de grupo e uma extracção de coordenada.
Se o ramo de topo transbordar, cria-se uma nova raiz e mais uma subida; o custo
continua linear em \(d\).

Os pontos geradores fixos são preparados uma vez, e tabelas de base fixa são
uma optimização possível. O que se pré-calcula de facto é o estado conhecido:
resumos dos ramos e fronteira activa; os contributos de folhas ainda
desconhecidas não podem ser pré-calculados. Conservar os ramos necessários às
testemunhas acrescenta armazenamento e E/S, separados da aritmética acima. Ao
verificar um gasto, o nó verifica a prova contra a raiz guardada; não reconstrói
a árvore. A Secção~\ref{sec:fcmp-tree-growth} formaliza esta actualização.
...
secreta \(i_\ell\); dentro desta linha, o \(\ell\) à esquerda de \(X\) já
fixa o nível, e escrevemos apenas \(i\). Quando
\(\ell\geq1\), esse filho é um ponto e re-randomiza-se como
\begin{align}
 {}_{\ell}\widehat X_i
   ={}_{\ell}X_i+\delta_\ell H_\ell.
...
é precisamente o \({}_{\ell+1}X_i\) seleccionado na camada de
cima.
...
\({}_{\ell}\widehat X_i\) é uma re-randomização desse ponto.

O compromisso vectorial demonstra que \({}_{\ell}X_i\) ocupa uma
posição do nó comprometido. O cegamento impede a comparação directa de
\({}_{\ell}\widehat X_i\) com os filhos públicos. A propriedade de
...
\begin{align*}
 ...
 &i&=2,
 &\mathcal B_0&\xrightarrow{\operatorname{Com}_{0\to1}}P_1,\\
 \mathcal B_1
...
\begin{align*}
 \underbrace{\operatorname{Com}_{0\to1}(\mathcal B_0)}_
   ...
   =\boxed{P_1}=
 \underbrace{{}_{1}X_1=\mathcal B_1[1]}_
...
\begin{tikzpicture}[
 ...
 \draw[->,very thick,blue!65!black] (l6) --
   node[left,align=right]{posição \(2\)\\em \(\mathcal B_0\)} (p1);
 \draw[->,very thick,blue!65!black] (p1) --
   node[left,align=right]{posição \(1\)\\em \(\mathcal B_1\)} (root);
...
O vínculo entre as duas verificações é a igualdade central
\(\operatorname{Com}_{0\to1}(\mathcal B_0)=P_1=\mathcal B_1[1]\), não uma
igualdade entre as máscaras. O circuito usa exactamente
\begin{align*}
 \mathcal B_0[2]&={}_{0}X_2=L_6
 &\longleftrightarrow\quad
 {}_{0}\widetilde X_2&=\widetilde L_6,\\
 \operatorname{Com}_{0\to1}(\mathcal B_0)
   &=P_1=\mathcal B_1[1]={}_{1}X_1
 &\longleftrightarrow\quad
 {}_{1}\widehat X_1&=P_1+\delta_1H_1,\\
 \operatorname{Com}_{1\to2}(\mathcal B_1)
   &=\mathcal R_{\rm tree}
 &\longleftrightarrow\quad
 \widehat{\mathcal R}&=\mathcal R_{\rm tree}+\rho H_2.
...
que precisa de subir: a saída \(P_1\) do primeiro nível é a entrada
\({}_{1}X_1=\mathcal B_1[1]\) do segundo.
...
No exemplo anterior, o provador quer demonstrar que \(L_6\) ocupa a posição
\(2\) do ramo \(\mathcal B_0\) e que os pais resultantes chegam à raiz. Não
...
\begin{align*}
 ...
 {}_{1}\widehat X_1
   &\longleftrightarrow
     \operatorname{Com}_{0\to1}(\mathcal B_0)
       =\boxed{P_1=\mathcal B_1[1]={}_{1}X_1},\\
 \widehat{\mathcal R}
   &\longleftrightarrow
     \operatorname{Com}_{1\to2}(\mathcal B_1)
       =\mathcal R_{\rm tree}.
...
\end{align*}
As duas afirmações existenciais poderiam não formar o mesmo caminho. As
...
\begin{tikzpicture}[node distance=1.60cm]
 ...
 \node (one) [pathbox,below of=leaf,yshift=-.05cm,fill=blue!12]
   {\(\mathcal B_0[2]={}_{0}X_2=L_6=(O_6,I_6,C_6)\)\\
    \(P_1=\operatorname{Com}_{0\to1}(\mathcal B_0)\)};
...
\({}_{\ell}\widehat X_i\) não permite testar eficientemente qual
