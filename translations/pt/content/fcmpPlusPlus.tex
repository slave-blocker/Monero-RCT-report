...
Nas camadas superiores de Curve Trees, cada filho é um ponto. Chamaremos
\(X_i\) ao filho genérico que ocupa a posição \(i\) de um pai:
\begin{align*}
 X_i=(x_i,y_i)
\end{align*}
...
instanciação do Monero, cada folha é a saída
\(L_i=(O_i,I_i,C_i^{\rm amt})\); o sobrescrito distingue o compromisso de
montante do nó de árvore \(C_j\). Cada \(C_j\) compromete as seis coordenadas
dos quatro \(L_i\) que contém. A raiz compromete as coordenadas dos quatro
\(C_j\):
...
\begin{align*}
 \underbrace{L_6=(O_6,I_6,C_6^{\rm amt})}_{\text{posição secreta }2}
 \quad\in\quad
 \underbrace{C_1}_{\substack{\text{pai de }L_6\\
                              \text{filho da raiz}}}
 \quad\in\quad
 \underbrace{\mathcal R_{\rm tree}}_{\substack{C_1\text{ na}\\
                                                \text{posição secreta }1}}.
\end{align*}
...
Congelemos um único nível. \(X_j\) significa apenas «o filho na posição
\(j\) deste pai»:
\begin{align*}
 \text{primeiro nível:}&\quad X_j=L_j=(O_j,I_j,C_j^{\rm amt}),\\
 \text{níveis seguintes:}&\quad X_j=C_j.
\end{align*}

O pai compromete \((X_0,\ldots,X_{m-1})\). O provador conhece um índice
secreto \(i\) e apresenta uma versão fresca do filho escolhido. Quando
\(X_i=(x_i,y_i)\) é um ponto de uma camada superior, re-randomiza-o como
\begin{align}
 \widehat X_i=X_i+\delta H_{\rm outro}.
...
\begin{align}
 ...
 \widehat X_i&=(x_i,y_i)+\delta H_{\rm outro}.
...
\begin{tikzpicture}[
 ...
 treebox/.style={rectangle,rounded corners,draw=black,thick,
   minimum width=5.5cm,minimum height=1.0cm,text centered,align=center},
 maskbox/.style={rectangle,rounded corners,draw=black,
   minimum width=6.2cm,minimum height=1.0cm,text centered,align=center,
   fill=yellow!20},
 every node/.style={font=\small}]
 \node[treebox,fill=blue!12] (root)
   {\(\mathcal R_{\rm tree}=\operatorname{Com}(C_0,C_1,C_2,C_3)\)};
 \node[treebox,below=of root,fill=green!18] (c1)
   {\(C_1=\operatorname{Com}(L_4,L_5,L_6,L_7)\)\\
    \scriptsize pai em baixo; o mesmo filho em cima};
 \node[treebox,below=of c1,fill=green!18] (l6)
   {\(L_6=(O_6,I_6,C_6^{\rm amt})\)};
 \node[maskbox,right=of root] (rhat)
   {\(\widehat{\mathcal R}=\mathcal R_{\rm tree}+\rho H\)};
 \node[maskbox,right=of c1] (chat)
   {\(\widehat C_1=C_1+\delta_1H\)};
 \node[maskbox,right=of l6] (ltilde)
   {\(\widetilde L_6=\operatorname{Rerand}_{\mathbf r}
      (O_6,I_6,C_6^{\rm amt})\)};
 ...
 \draw[->,very thick,blue!65!black] (l6) --
   node[left,align=right]{posição \(2\)\\escondida} (c1);
 \draw[->,very thick,blue!65!black] (c1) --
   node[left,align=right]{posição \(1\)\\escondida} (root);
 \draw[->,thick,orange!80!black] (l6) -- (ltilde);
 \draw[->,thick,orange!80!black] (c1) -- (chat);
 \draw[->,thick,orange!80!black] (root) -- (rhat);
\end{tikzpicture}
\end{center}

Na linha da folha, \(\operatorname{Rerand}_{\mathbf r}\) abrevia
as re-randomizações separadas de \(O_6,I_6,C_6^{\rm amt}\) e o compromisso
...
e que a versão pública \(\widehat X_i\) é uma re-randomização desse ponto.

O compromisso vectorial demonstra que \(X_i\) ocupa uma posição do nó
comprometido. O cegamento impede a comparação directa de \(\widehat X_i\) com
os filhos públicos. A propriedade de conhecimento zero impede que o
verificador aprenda \(i\), \(\delta\) ou a abertura completa do pai.
...
\begin{align*}
 \widetilde L_6
   &:=\operatorname{Rerand}_{\mathbf r}(O_6,I_6,C_6^{\rm amt}),\\
 \widehat C_1&=C_1+\delta_1H_1,\\
 \widehat{\mathcal R}
...
\begin{align*}
 \widetilde L_6
   &\longleftrightarrow
     L_6=(O_6,I_6,C_6^{\rm amt})\in\boxed{C_1},\\
 \widehat C_1
   &\longleftrightarrow
     \boxed{C_1}\in\mathcal R_{\rm tree},\\
 \widehat{\mathcal R}
   &\longleftrightarrow
     \mathcal R_{\rm tree}.
\end{align*}
...
representação pública. Como \(\delta\)
é uniforme, \(\widehat X\) não permite testar eficientemente qual \(X\) foi
