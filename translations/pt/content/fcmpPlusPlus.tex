...
mesmo \(r_i\) foi usado para ofuscar a base \(I\) da imagem de chave. São três
objectos, três ciclos de vida e três razões de segurança.

\subsection{Prova de conhecimento do factor de ofuscação da raiz}

Usaremos três nomes para três tipos diferentes:
\begin{align*}
 \underbrace{\rho}_{\text{factor de ofuscação: escalar}}
 \xrightarrow{\ \cdot H\ }
 \underbrace{\rho H}_{\text{termo de ofuscação: ponto}}
 \xrightarrow{\ +\mathcal R_{\rm tree}\ }
 \underbrace{\widehat{\mathcal R}}_
             {\text{representação ofuscada: ponto}}.
\end{align*}

Suponhamos que dois pontos públicos diferem por um múltiplo conhecido de uma
base \(H\):
\begin{align}
 X=\widehat{\mathcal R}-\mathcal R_{\rm tree}
   =\rho H.
...
apenas o conhecimento do respectivo factor de ofuscação. Schnorr autentica a
...
caminho transporta um ponto completo; o ramo do pai guarda apenas o seu
rótulo escalar \(x\):
\begin{align*}
 \underbrace{{}_{\ell}X_j=({}_{\ell}x_j,{}_{\ell}y_j)}_
             {\text{ponto que sobe e pode ser ofuscado}},
 \qquad
 \underbrace{\xi_{\ell,j}:=x({}_{\ell}X_j)={}_{\ell}x_j}_
             {\text{escalar guardado no pai}},
 \qquad \ell\geq1.
\end{align*}
O índice à esquerda marca o nível; o índice à direita marca a posição local
dentro de um pai. Na construção actual, um ramo superior é portanto
\begin{align*}
 \underbrace{\mathcal B_\ell=
   (\xi_{\ell,0},\ldots,\xi_{\ell,m-1})}_
   {\substack{\text{um ramo: }m\text{ rótulos}\\
              \text{dos irmãos no nível }\ell}}
 \quad\xrightarrow{\ \operatorname{Com}_{\ell\to\ell+1}\ }\quad
 \underbrace{P^{(\ell+1)}}_{\text{um ponto no nível }\ell+1}.
\end{align*}
Na primeira camada há a única excepção tipográfica: as entradas de
\(\mathcal B_0\) são folhas completas \(L_j=(O_j,I_j,C_j)\), não rótulos
\(\xi\).

Um ramo \(\mathcal B_\ell\) é a lista ordenada das entradas associadas aos
filhos de \emph{um} pai; não é o nível inteiro nem o trajecto da folha à raiz.
Para \(\ell\geq1\), essas entradas são rótulos escalares, e os colchetes
escolhem o rótulo de uma posição:
\begin{align}
 \boxed{\mathcal B_\ell[i]=\xi_{\ell,i}=x({}_{\ell}X_i)}.
 \label{eq:fcmp-branch-slot}
\end{align}
O circuito conserva também o ponto completo \({}_{\ell}X_i\): precisa dele
...
\begin{align}
 P^{(\ell+1)}
   =\operatorname{Com}_{\ell\to\ell+1}(\mathcal B_\ell)
   =J^{(\ell+1)}_{\rm init}
    +\sum_{j=0}^{m-1}\xi_{\ell,j}G^{(\ell+1)}_j.
...
\begin{align*}
 ...
 \xi_{\ell,0}G_0+\xi_{\ell,1}G_1
 &=\xi_{\ell,1}G_1+\xi_{\ell,0}G_0,\\
 \text{permutar os filhos:}\quad
 \xi_{\ell,0}G_0+\xi_{\ell,1}G_1
 &\underset{\text{em geral}}{\ne}
 \xi_{\ell,1}G_0+\xi_{\ell,0}G_1.
\end{align*}
...
baralha \(\mathcal B_\ell\): a prova esconde \(i_\ell\), não reordena o ramo.

Um caminho atravessa níveis. O compromisso de um ramo produz o ponto que sobe;
o ramo seguinte guarda o seu rótulo \(x\) numa posição possivelmente
diferente:
\begin{align}
 \boxed{
 P^{(\ell+1)}={}_{\ell+1}X_{i_{\ell+1}}
 \quad\Longrightarrow\quad
 \mathcal B_{\ell+1}[i_{\ell+1}]=x(P^{(\ell+1)})}.
...
Reservemos maiúsculas para os pontos que sobem e minúsculas para os rótulos
\(x\) guardados no ramo seguinte:
\begin{align*}
 L_r&=(O_r,I_r,C_r),\qquad 0\leq r<27,\\
 P_j&=\operatorname{Com}_{0\to1}(L_{3j},L_{3j+1},L_{3j+2}),
 &p_j&:=x(P_j),&0&\leq j<9,\\
 Q_k&=\operatorname{Com}_{1\to2}(p_{3k},p_{3k+1},p_{3k+2}),
 &q_k&:=x(Q_k),&0&\leq k<3,\\
 \mathcal R_{\rm tree}
 &=\operatorname{Com}_{2\to3}(q_0,q_1,q_2).
\end{align*}
Cada folha fornece três pontos \((O,I,C)\), duas coordenadas por ponto. As
setas \(P_j\mapsto p_j\) e \(Q_k\mapsto q_k\) são extracções de \(x\), não
mudanças de nome:
\begin{align*}
 \boxed{P_j\text{ é um ponto}}
 &\xrightarrow{\ x\ }
 \boxed{p_j=x(P_j)\text{ é um escalar}},\\
 \boxed{Q_k\text{ é um ponto}}
 &\xrightarrow{\ x\ }
 \boxed{q_k=x(Q_k)\text{ é um escalar}}.
\end{align*}
...
[\(\mathcal R_{\rm tree}\),fill=blue!12
  [{\(Q_0\)\\\(\vdots\)\\{\scriptsize\(L_0,\ldots,L_8\)}},fill=black!10,
    draw=black!55,dashed,edge={draw=black!40,dashed}]
  [{\(Q_1\)\\\(\vdots\)\\{\scriptsize\(L_9,\ldots,L_{17}\)}},fill=black!10,
    draw=black!55,dashed,edge={draw=black!40,dashed}]
  [\(Q_2\),fill=green!18,edge={very thick,blue!65!black}
    [{\(P_6\)\\\(\vdots\)\\{\scriptsize\(L_{18},\ldots,L_{20}\)}},fill=black!10,
      draw=black!55,dashed,edge={draw=black!40,dashed}]
    [\(P_7\),fill=green!18,edge={very thick,blue!65!black}
      [\(L_{21}\),fill=yellow!25,edge={very thick,blue!65!black}]
      [\(L_{22}\)]
      [\(L_{23}\)]]
    [{\(P_8\)\\\(\vdots\)\\{\scriptsize\(L_{24},\ldots,L_{26}\)}},fill=black!10,
      draw=black!55,dashed,edge={draw=black!40,dashed}]]]
\end{forest}
\end{center}

Cada grupo de irmãos determina um ramo local; acima do nível \(0\), esse ramo
guarda os seus rótulos \(x\), não os pontos completos. A cadeia de arestas
...
\end{align*}
\(L_{21}\) ocupa a posição \(0\) no seu ramo, o ponto \(P_7\) fornece o
rótulo da posição \(1\) no ramo seguinte e \(Q_2\) fornece o rótulo da posição
\(2\) junto à raiz. As quatro linhas são
\begin{align*}
 \mathcal B_0&=(L_{21},L_{22},L_{23}),
 &\mathcal B_0[0]&=L_{21}=(O_{21},I_{21},C_{21}),\\
 \operatorname{Com}_{0\to1}(\mathcal B_0)&=P_7,
 &\mathcal B_1&=(p_6,\boxed{p_7=x(P_7)},p_8),\\
 \operatorname{Com}_{1\to2}(\mathcal B_1)&=Q_2,
 &\mathcal B_2&=(q_0,q_1,\boxed{q_2=x(Q_2)}),\\
...
\(p_7=x(P_7)=\mathcal B_1[1]\). Do mesmo modo,
\(q_2=x(Q_2)=\mathcal B_2[2]\). A prova esconde as três posições
...
\(L_{21}\) já existem e chega a saída real \(L_{22}\). Apenas uma posição do
ramo activo muda:
\begin{align*}
\begin{array}{r@{\qquad}c@{\qquad}c@{\qquad}c@{\qquad}l}
 \text{posição global da folha}&21&\boxed{22}&23&\text{resumo}\\
 \mathcal B_0\text{ antes}&L_{21}&\square&\square&\Longrightarrow P_7\\
 \mathcal B_0\text{ depois}&L_{21}&L_{22}&\square&\Longrightarrow P_7'
\end{array}
\end{align*}
Na linha ``antes'', \(L_{22}\) e \(L_{23}\) ainda não existem; na linha
``depois'', apenas \(L_{22}\) passou a existir. Os \(\square\) não são saídas
fictícias usadas numa transacção: são enchimento zero, contribuem
\(0\mathbin{\cdot}G_j=\mathcal O\) e não podem ser seleccionados nem gastos.
A saída real substitui \(\square\) pela folha completa
\(L_{22}=(O_{22},I_{22},C_{22})\).
...
\end{align*}
Este não é o caminho secreto anterior \((0,1,2)\) de \(L_{21}\). A nova
folha escolhe a posição \(1\) dentro de \(\mathcal B_0\), mas partilha com
\(L_{21}\) as posições superiores \(1\) e \(2\); por isso actualiza os mesmos
antepassados \(P_7\) e \(Q_2\).

O ponto mudou, logo muda o seu rótulo
\(p_7=x(P_7)\mapsto p_7'=x(P_7')\). Esse único escalar ocupa a mesma posição
\(1\) no ramo seguinte:
\begin{align*}
\begin{array}{r@{\qquad}c@{\qquad}c@{\qquad}c@{\qquad}l}
 \text{posição dentro de }Q_2&0&\boxed{1}&2&\text{resumo}\\
 \mathcal B_1\text{ antes}&p_6&p_7&\square&\Longrightarrow Q_2\\
 \mathcal B_1\text{ depois}&p_6&p_7'&\square&\Longrightarrow Q_2'
\end{array}
\end{align*}
Finalmente, \(q_2=x(Q_2)\mapsto q_2'=x(Q_2')\) altera a posição \(2\) do
ramo da raiz:
\begin{align*}
\begin{array}{r@{\qquad}c@{\qquad}c@{\qquad}c@{\qquad}l}
 \text{posição dentro da raiz}&0&1&\boxed{2}&\text{raiz}\\
 \mathcal B_2\text{ antes}&q_0&q_1&q_2&\Longrightarrow\mathcal R_{\rm tree}\\
 \mathcal B_2\text{ depois}&q_0&q_1&q_2'
...
\begin{align*}
 ...
 \xrightarrow{\ x\ }p_7\mapsto p_7'\\
 &\Longrightarrow Q_2\mapsto Q_2'
 \xrightarrow{\ x\ }q_2\mapsto q_2'
...
alterada. Neste exemplo de profundidade três, a chegada de \(L_{22}\) custa
\begin{align*}
 \underbrace{2+2+2}_{O_{22},\ I_{22},\ C_{22}}
 +\underbrace{1}_{p_7'\text{ em }\mathcal B_1}
 +\underbrace{1}_{q_2'\text{ em }\mathcal B_2}
...
\begin{align*}
 ...
 &=\Theta(d)=\Theta(\log_mN),
   \qquad\text{em vez de reconstruir }\Theta(N).
\end{align*}
Aqui são três actualizações de hash/adições de grupo e duas extracções de
coordenada \(x\). Ainda não há transporte: \(L_{23}\) completa \(P_7\),
\(L_{24}\) abre o irmão \(P_8\), e só \(L_{27}\) excede esta árvore de
...
rótulos \(x\). Ao esgotar a capacidade do topo, acrescenta-se uma nova raiz
...
\begin{align*}
 \mathcal B_0&=(L_{21},L_{22},L_{23}),&i&=0,\\
 \mathcal B_1&=(p_6,p_7,p_8),&i&=1,\\
 \mathcal B_2&=(q_0,q_1,q_2),&i&=2.
\end{align*}
O índice \(\ell\) identifica o nível; \(i_\ell\) escolhe uma das posições
locais. Em geral e neste exemplo,
\begin{align*}
...
Congelemos um nível superior \(\ell\geq1\). O ramo compromete os rótulos
\((\xi_{\ell,0},\ldots,\xi_{\ell,m-1})\). O provador conhece a posição
secreta \(i\), o ponto completo \({}_{\ell}X_i\) e a igualdade
\(\mathcal B_\ell[i]=\xi_{\ell,i}=x({}_{\ell}X_i)\). Publica uma versão
...
Aqui \(H_\ell\) é a base de ofuscação na curva onde vive esse ponto. A
declaração de um nível tem duas metades:
\begin{align}
 \widehat P^{(\ell+1)}
   &=\underbrace{J^{(\ell+1)}_{\rm init}
     +\sum_{j=0}^{m-1}\xi_{\ell,j}G^{(\ell+1)}_j}_{P^{(\ell+1)}}
     +\delta_{\ell+1}H_{\ell+1},
 \label{eq:curve-tree-open-parent}\\
 {}_{\ell}\widehat X_i
   &=({}_{\ell}x_i,{}_{\ell}y_i)
     +\delta_\ell H_\ell,
 &\mathcal B_\ell[i]&={}_{\ell}x_i.
...
ponto seleccionado e liga a sua coordenada \(x\) à posição escolhida. Pela
equação \eqref{eq:curve-tree-parent-becomes-child}, o ponto pai sobe; o seu
\(x\) entra no ramo seguinte.
...
\begin{tikzpicture}[
 ...
 \node[treebox,below=of root,fill=green!18] (q2)
   {\(Q_2={}_{2}X_2=\operatorname{Com}_{1\to2}(\mathcal B_1)\)\\[-1mm]
    \(\mathcal B_2[2]=q_2=x(Q_2)\)};
 \node[treebox,below=of q2,fill=green!18] (p7)
   {\(P_7={}_{1}X_1=\operatorname{Com}_{0\to1}(\mathcal B_0)\)\\[-1mm]
    \(\mathcal B_1[1]=p_7=x(P_7)\)};
 \node[treebox,below=of p7,fill=green!18] (l21)
   {\(\mathcal B_0[0]=L_{21}=(O_{21},I_{21},C_{21})\)};
 \node[maskbox,right=of root] (rhat)
   {\(\widehat{\mathcal R}=\mathcal R_{\rm tree}+\rho H_3\)};
 \node[maskbox,right=of q2] (qhat)
   {\({}_{2}\widehat X_2=\widehat Q_2=Q_2+\delta_2H_2\)};
 \node[maskbox,right=of p7] (phat)
   {\({}_{1}\widehat X_1=\widehat P_7=P_7+\delta_1H_1\)};
 \node[maskbox,right=of l21] (ltilde)
   {\({}_{0}\widetilde X_0=\widetilde L_{21}\)\\[-1mm]
    \(=\operatorname{Rerand}_{\mathbf r}(O_{21},I_{21},C_{21})\)};
...
A pergunta certa não é ``a folha ofuscada ainda se parece com \(L_{21}\)?''.
...
\begin{align*}
 &\underbrace{\mathcal B_0[0]
      =L_{21}=(O_{21},I_{21},C_{21})}_{\text{folha escondida na árvore}}
 \xleftarrow[\quad\Pi_{\rm FCMP}\quad]
   {\ r_o,r_i,r_j,r_c,\ldots\ }
 \underbrace{(\widetilde O,\widetilde I,R_{\rm fcmp},\widetilde C)}_
             {\text{a tupla pública comum}},\\[.7em]
 &\underbrace{(\widetilde O,\widetilde I,R_{\rm fcmp},\widetilde C)}_
             {\text{a mesma tupla pública}}
 \xrightarrow[\quad\Pi_{\rm SAL}\quad]
   {\ x,\widetilde y,r_i,r_j;\ z=xr_i,\ldots\ }
 \underbrace{L=xI_{21}}_{\text{autorização e marcador}}.
\end{align*}
À esquerda, FCMP recupera \(O_{21},I_{21},C_{21}\) com
\(r_o,r_i,r_c\), abre separadamente
\(R_{\rm fcmp}=r_iV+r_jT\) e demonstra que a tupla recuperada ocupa uma folha
escondida da árvore. O escalar \(r_j\) não se subtrai a um componente da folha:
oculta \(r_i\) dentro de \(R_{\rm fcmp}\). À direita, SAL demonstra
conhecimento da chave \(x\) usando a mesma tupla pública. Não há uma segunda
cópia que possa ser ligada a outra folha.
...
\begin{align*}
 ...
 \boxed{\text{SAL}}
   &:\quad \text{prova conhecimento da chave \(x\) compatível com a mesma tupla}.
\end{align*}
Qualquer pessoa pode somar múltiplos aleatórios a pontos públicos. Essa pessoa
conseguiria produzir uma nova aparência, mas não completar a SAL sem \(x\).
Aqui, \(x(P_7)\) e \(x(Q_2)\) denotam coordenadas de pontos; \(x\) sem
argumento é a chave privada de utilização única. É esta chave que fica
invariável. Para o ver, usemos a forma geral da chave de saída:
\begin{align*}
 O_{21}=xG+yT
 \quad\longmapsto\quad
 \widetilde O=O_{21}+r_oT=xG+(y+r_o)T,
 \qquad\boxed{x\text{ não mudou}}.
\end{align*}
Mais tarde, a SAL exige esse mesmo \(x\). Também exige \(z=xr_i\), de modo que
o termo que ofuscou \(I_{21}\) desaparece apenas na equação autorizada:
\begin{align*}
 \underbrace{L}_{\substack{\text{marcador de gasto}\\
                            \text{não a folha }L_{21}}}
 =x\widetilde I-zU
 =x(I_{21}+r_iU)-xr_iU
 =xI_{21}.
\end{align*}
O compromisso \(R_{\rm fcmp}\) impede usar um \(r_i\) na abertura FCMP e outro
na multiplicação \(z=xr_i\). Portanto, FCMP responde ``uma folha real da
...
Na linha \(\ell=0\),
\({}_{0}\widetilde X_0=\widetilde L_{21}\) abrevia
as re-randomizações separadas de \(O_{21},I_{21},C_{21}\) e o compromisso
...
\(i_\ell\) e os escalares de ofuscação. O circuito verifica a abertura do
pai, que a posição escolhida existe, que o ponto está na curva e que o seu
\(x\) é exactamente o rótulo dessa posição. A ofuscação impede a comparação
directa de \({}_{\ell}\widehat X_i\) com os pontos públicos. A propriedade de
conhecimento zero impede que o verificador aprenda \(i_\ell\),
\(\delta_\ell\) ou a abertura completa do pai.
...
\begin{align*}
 \mathcal B_0&=(\boxed{L_{21}},L_{22},L_{23}),
 &i&=0,
...
\begin{align*}
 \mathcal R_{\rm tree}
   =J^{(3)}_{\rm init}+q_0G^{(3)}_0+q_1G^{(3)}_1+q_2G^{(3)}_2,
 \qquad \mathcal B_2[2]=q_2=x(Q_2).
\end{align*}

O diagrama anterior já desenha a escada completa; não precisamos de a repetir.
Os termos de ofuscação ficam nas setas horizontais, enquanto os dois encaixes
\(x(P_7)=p_7\) e \(x(Q_2)=q_2\) mantêm o caminho na coluna esquerda. O
circuito usa exactamente
\begin{align*}
 \mathcal B_0[0]&=L_{21}
 &\longleftrightarrow\quad
 {}_{0}\widetilde X_0&=\widetilde L_{21},\\
 \operatorname{Com}_{0\to1}(\mathcal B_0)
   &=P_7,\qquad x(P_7)=p_7=\mathcal B_1[1]
 &\longleftrightarrow\quad
 {}_{1}\widehat X_1&=P_7+\delta_1H_1,\\
 \operatorname{Com}_{1\to2}(\mathcal B_1)
   &=Q_2,\qquad x(Q_2)=q_2=\mathcal B_2[2]
 &\longleftrightarrow\quad
 {}_{2}\widehat X_2&=Q_2+\delta_2H_2,\\
...
precisam de subir. Os factores de ofuscação são independentes; o que se repete
entre níveis é o ponto canónico e o seu rótulo \(x\), não esses factores.
...
\(L_{21}\to P_7\to Q_2\to\mathcal R_{\rm tree}\). Não publica a folha, os
dois pontos internos nem os índices \((0,1,2)\). Em vez disso, produz quatro
representações ofuscadas:
\begin{align*}
 {}_{0}\widetilde X_0=\widetilde L_{21}
   &:=\operatorname{Rerand}_{\mathbf r}(O_{21},I_{21},C_{21}),\\
 {}_{1}\widehat X_1=\widehat P_7
   &:=P_7+\delta_1H_1,\\
 {}_{2}\widehat X_2=\widehat Q_2
   &:=Q_2+\delta_2H_2,\\
 \widehat{\mathcal R}
   &:=\mathcal R_{\rm tree}+\rho H_3.
\end{align*}
...
\begin{align*}
 ...
 {}_{1}\widehat X_1
   &\longleftrightarrow
     \operatorname{Com}_{0\to1}(\mathcal B_0)=P_7,
     \quad\boxed{x(P_7)=p_7=\mathcal B_1[1]},\\
 {}_{2}\widehat X_2
   &\longleftrightarrow
     \operatorname{Com}_{1\to2}(\mathcal B_1)=Q_2,
     \quad\boxed{x(Q_2)=q_2=\mathcal B_2[2]},\\
 \widehat{\mathcal R}
   &\longleftrightarrow
     \operatorname{Com}_{2\to3}(\mathcal B_2)
       =\mathcal R_{\rm tree}.
...
exactamente o rótulo \(x\) escolhido na linha seguinte. O único diagrama da
...
\begin{align*}
 X&=\widehat{\mathcal R}-\mathcal R_{\rm tree}=\rho H_3,\\
 R_{\rm sig}+cX&=sH_3.
\end{align*}
...
\begin{align*}
 ...
 \underbrace{\Pi_{\rm root}}_{\substack{\widehat{\mathcal R}
      -\mathcal R_{\rm tree}\\=\rho H_3\text{ e conheço }\rho}}
 \quad\Longrightarrow\quad
...
representação pública. Como \(\delta_\ell\) é uniforme em \(\mathbb Z_q\) e
\(H_\ell\) gera o grupo de ordem prima,
\({}_{\ell}X_i+\delta_\ell H_\ell\) tem a mesma distribuição para
qualquer candidato \({}_{\ell}X_i\). O ponto ofuscado, sozinho, não oferece um
teste para \(i\); a propriedade de conhecimento zero garante que a prova que o
acompanha também não revela \(i\) nem \(\delta_\ell\).

Pontos ofuscados com factores independentes, sozinhos, também não demonstram
que formam um único caminho. O circuito abre cada representação no testemunho
e impõe os encaixes canónicos \(x(P_7)=p_7\) e \(x(Q_2)=q_2\). Os factores
continuam deliberadamente independentes; são essas igualdades, não uma
correlação entre factores, que mantêm a consistência entre níveis.
...
corrente de \texttt{monero-oxide}, porém, o fluxo é directamente
\begin{align*}
...
ordenada \(L_i=(O_i,I_i,C_i)\).

Os endereços de utilização única não desapareceram: em cada folha, \(O_i\) é
precisamente a chave pública de saída de utilização única e \(x_i\) é a chave
privada que a carteira recupera para a gastar. FCMP++ não substitui a derivação
desses endereços; substitui a lista explícita usada para provar pertença.
...
\end{align*}
onde \(x\) é a chave privada de utilização única, \(a\) a máscara do
compromisso de montante e \(b\) o montante. A imagem de chave que aparecerá ao
gastar é
\begin{align*}
...
\begin{itemize}
 ...
 \item somar \(r_cG\) muda apenas a máscara \(a\) do compromisso de montante
...
vinculasse \(r_i\) ao restante testemunho. \(R_{\rm fcmp}\) fornece esse
compromisso. Ele não revela \(r_i\), porque está ofuscado pelo termo aleatório
\(r_jT\), e não é o nonce da prova de Schnorr da raiz.
...
O termo de ofuscação \(r_iU\) desaparece, mas só desaparece correctamente se o
mesmo \(r_i\) usado na prova de pertença entrar em \(z=xr_i\). É agora que
...
\begin{align}
 ...
 \label{eq:fcmp-sal-p}
\end{align}
com um novo escalar de ofuscação \(r_p\). \(P\) é um compromisso a
...
Escrevemos agora as equações completas. O provador escolhe nonces aleatórios da prova
\begin{align*}
...
\subsection{Quarta linha: marcador determinístico}
...
\begin{align*}
 L=xI,
\end{align*}
independentemente dos novos escalares de ofuscação
...
re-randomização, e vê \(L\), que é determinístico. A SAL prova que, ao derivar
\(L\), o termo de ofuscação foi cancelado de modo consistente sem revelar o
ponto de que partiu.
...
\begin{enumerate}
 ...
 \(\widehat{\mathcal R}-\mathcal R_{\rm tree}=\rho H\), para um factor de
 ofuscação \(\rho\) conhecido pelo provador.
...
\end{align*}
Todos os pontos excepto \(L\) incorporam nonces ou novos termos de ofuscação,
...
\begin{tabular}{|c|c|c|}
 ...
 \hline
 \(U\) & \(\widetilde I-I=r_iU\), termo de ofuscação de \(I\) & \(\log_GU\)\\
 \hline
...
O \(\square\) é codificado internamente por zeros de enchimento; não é um
membro gastável. Na primeira camada, um membro válido tem duas coordenadas
para cada um dos três pontos não identidade \((O,I,C)\), e o circuito verifica
que esses pontos estão na curva. Nas camadas seguintes, a declaração
...
para o hash do ramo inferior. A construção chama genericamente
\begin{align*}
 h_j:=x(P_j)
\end{align*}
ao rótulo e compromete os \(h_j\) no pai. No exemplo ternário, esses mesmos
rótulos receberam nomes concretos \(p_j=x(P_j)\) e \(q_k=x(Q_k)\). Assim,
...
\begin{enumerate}
 ...
 novos termos de ofuscação. Com a mesma saída e chave, o marcador repete-se.
\end{enumerate}
...
\begin{tabular}{|l|p{5.1cm}|p{5.1cm}|}
 ...
 segredos de gasto e máscaras dos compromissos de montante ainda não
 revelados\\
 \hline
\end{tabular}
\end{center}
...
\begin{description}
 ...
 interna.
 \item[Raiz:] o provador conhece o factor de ofuscação que relaciona a raiz
 interna com \(\mathcal R_{\rm tree}\).
