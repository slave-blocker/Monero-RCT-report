...
\end{align*}
Cada \(P_j\) compromete as seis coordenadas das quatro folhas
\(L_{4j},\ldots,L_{4j+3}\). A raiz compromete as coordenadas dos quatro
\(P_j\):
...
já ocupa o primeiro lugar do ramo activo, a chegada de \(L_5\) percorre apenas
\begin{align*}
 \mathcal B_0:\quad
 (L_4,\boxed{\mathbf 0_6\mapsto L_5},\mathbf 0_6,\mathbf 0_6)
 &\Longrightarrow P_1\mapsto P_1',\\
 \mathcal B_1:\quad
 (P_0,\boxed{P_1\mapsto P_1'},0,0)
 &\Longrightarrow
 \mathcal R_{\rm tree}\mapsto\mathcal R'_{\rm tree}.
\end{align*}
Aqui \(\mathbf 0_6\) é um lugar vazio para as seis coordenadas de uma folha;
os zeros da segunda linha são lugares vazios para rótulos escalares. \(P_1\)
muda de valor na mesma posição \(1\). \(P_0\) e todo o seu ramo ficam em
memória, sem serem revisitados.

Pela linearidade, o compromisso guardado recebe apenas a diferença da posição
alterada. Na instanciação actual, uma saída nova fornece seis termos na camada
das folhas e um rótulo do hash em cada camada superior. Sem aumento de altura,
\begin{align*}
 \underbrace{6}_{(O,I,C)}
 +\underbrace{(d-1)}_{\text{um rótulo por camada superior}}
 &=d+5\quad\text{novos termos escalar--ponto},\\
 \boxed{\text{actualização criptográfica da raiz}}
 &=O(d)=O(\log N),\qquad\text{não }O(N).
\end{align*}
