...
Considere-se uma árvore com dezasseis folhas e quatro filhos por pai. As folhas
...
\]
O provador conhece um índice secreto \(i\) e quer apresentar o filho \(P_i\) sem
...
O compromisso vectorial demonstra que \(P_i\) ocupa uma posição do nó
comprometido. O cegamento impede a comparação directa de \(\widehat P\) com
...
A instanciação FCMP++ usa duas curvas chamadas Selene e Helios para a alternância entre curvas. A tupla original do Monero vive na curva de saída, Ed25519; o
...
\end{align*}
Se falhar, reamostra-se. Não se fornece ao verificador uma inversão de zero e não se confunde uma falha rara de completude com uma falha de
solidez.
...
os dados necessários ao gasto de uma saída. Uma chave pública sozinha não chega:
...
pseudo-compromissos ao equilíbrio da transacção. A hash comum impede reutilizar a SAL numa transacção diferente.
