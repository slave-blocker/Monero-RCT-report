...
blocos, o endereço partilhado parece um endereço Monero e a transacção final
parece uma transacção RingCT vulgar. A política \(M\)-de-\(N\) vive nas
chaves, nos dados conservados pelas carteiras e no protocolo interactivo que
produz a assinatura. A lista de blocos não anuncia se o dinheiro pertence a
uma pessoa, a uma empresa ou a três pessoas que ainda estão a decidir quem
guardou o cabo USB.
...
\hline
Multi-assinatura FCMP++ ou Carrot & Não implementada nesta revisão &
É possível em princípio, mas o construtor examinado assina CLSAG; uma
integração precisa de um provador SAL distribuído e de suporte para as chaves
Carrot. \\
\hline
\end{tabular}
\end{center}
...
interactivo, não como uma simples caixa com duas palavras-passe.
...
recusar uma proposta, apagar os seus dados ou abandonar a conversa a meio.
Multi-assinatura reduz o número de participantes indispensáveis; não converte
um adversário num colega prestável.
...
montantes e observar o histórico da carteira. O limiar protege a autoridade de
\emph{gasto}, não a privacidade interna entre os membros do grupo.
...
incompleto de mensagens pode produzir uma conta inconsistente ou permitir que
um participante mantenha os restantes reféns de uma chave que só ele conhece.
...
\begin{align*}
 A+B=T,
\end{align*}
cuja chave privada \(t\) é conhecida por Bob. A cerimónia pode ter actas,
assinaturas e uma mesa muito solene; a álgebra não se deixa impressionar.
Este é um ataque de cancelamento de chaves.
...
problema criptográfico. A ordenação é essencial: sem ela, duas carteiras
podiam ter exactamente as mesmas chaves e, ainda assim, passar a tarde a
discutir qual delas estava avariada.
...
derivam a mesma parcela e qualquer um consegue usá-la. Isto é exactamente o
que «1-de-\(N\)» diz nas letras pequenas; não oferece protecção contra um
membro que decida gastar sozinho.
...
aplicável, o termo do subendereço; os \(x_j\) são as parcelas distintas da
...
Uma resposta de um nó remoto não confiável acerca do estado de gasto pode ser
...
\begin{itemize}
 ...
 \item o termo comum derivado da chave de vista e o ajuste de subendereço;
...
A chave privada da transacção é derivada de entropia fresca e das imagens de
chave das entradas. Isto evita que propostas repetidas criem as mesmas chaves
de saída de uso único e acabem por queimar fundos destinados a transacções
diferentes.
...
criptograficamente válida pode pagar com grande rigor matemático ao endereço
errado.
...
Em termos operacionais, uma sessão saudável segue esta sequência:
\begin{enumerate}
...
estado desta conversa na carteira de linha de comandos
...
entendidas como recuperação do mesmo participante, não como mais um voto no
limiar.
...
e a raiz forneceria a chave pública de gasto. Também se poderia imaginar que
...
\textbf{Resposta curta: é possível em princípio, mas não está implementada
na revisão estudada.} FCMP++ separa a prova de pertença à cadeia da prova SAL
de autorização e ligação \cite{fcmp-plus-plus-paper}. Um coordenador pode
construir o caminho de pertença sem conhecer a chave de gasto inteira. A
cooperação \(M\)-de-\(N\) concentra-se na imagem de chave e na SAL.
...
um esboço, não uma análise de segurança.
...
multi-assinatura \cite{carrot-specification}. Uma implementação nativa terá de
repartir directamente as duas componentes. A vista das despesas pode, em
contrapartida, actualizar o saldo sem activar tão frequentemente as parcelas
privadas de gasto \cite{monero-fcmp-introduction}.

Na cadeia ver-se-iam apenas uma prova de pertença, uma SAL e \(L\), nunca
\(M\), \(N\) ou os signatários. No código consultado, porém, o construtor
multi-assinatura termina numa CLSAG: não há comandos, mensagens nem provador
SAL distribuído. Logo, \emph{é possível, mas ainda não está disponível}.
