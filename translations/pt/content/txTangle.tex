...
\[
 \delta_u=\sum_{v\ne u}s_{u,v}
\]
e adiciona \(\delta_u\) à máscara de um dos seus pseudo-compromissos; isto é,
adiciona \(\delta_uG\) ao próprio pseudo-compromisso. Então
\begin{align}
 \sum_{u=1}^{N}\delta_u
...
\begin{enumerate}
 ...
 \item \emph{Vincular a vista da sessão.} Um anfitrião pode entregar listas
 diferentes a participantes diferentes. A defesa histórica vincula cada chave
 pública de transacção ao conjunto canónico de identidades
 \(\mathbb S_{\rm id}\) e à lista canónica das chaves originais
 \(\mathbb S_R^{(0)}\), com uma separação de domínio própria \(T_{\rm TT}\):
 \[
  R_t=
  \mathcal H_n(T_{\rm TT},\mathbb S_{\rm id},\mathbb S_R^{(0)},R_t^{(0)})
  \,R_t^{(0)},
  \qquad R_t^{(0)}=r_tG.
 \]
 Participantes que receberam listas diferentes obtêm chaves diferentes e não
 conseguem concluir a mesma transacção. Isto detecta a equivocação entre
 participantes honestos; não impede o anfitrião de preencher todas as outras
 identidades de uma sala e executar um ataque de poluição. Numa implementação
 moderna, a vinculação teria de abranger ainda a versão, a sessão, todas as
 mensagens e a ordem canónica, e não apenas estas chaves.
\end{enumerate}
...
é positivo com a convenção da Equação~\eqref{eq:txtangle-global-balance}; se se
inverter essa convenção, invertem-se ambos os lados. O invariante indispensável
é que a soma final das máscaras seja zero.
